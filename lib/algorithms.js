...
// certainty.
exports.updateRecommendationsFor = function(userId, options, cb){
      ...
      if (setsToUnion.length > 0){
        ...
        client.sunionstore(setsToUnion, function(err) {
          ...
          client.sdiff(tempAllLikedSet, Key.userLikedSet(userId), Key.userDislikedSet(userId), function(err, notYetRatedItems){
            ...
            if (options.recommendableItems) {
              notYetRatedItems = notYetRatedItems.filter(id => options.recommendableItems.indexOf(id) >= 0)
            }
